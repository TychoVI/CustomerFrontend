                ...
                <CardContent>
                    ...
                    <div>
                        {props.data.data.ingredients.map((ingredient, index) => (
                            index === 0 ? <FormControlLabel disabled control={<Checkbox checked name="checked"/>}
                                                            label={ingredient.name}/>
                                : <FormControlLabel control={<Checkbox name="checked"/>} label={ingredient.name}/>
                        ))}
                    </div>
                </CardContent>
            </Collapse>
            <Icon fontSize="large" color="primary">add_circle</Icon>
            <Icon fontSize="large" color="primary">remove_circle</Icon>
            <div>{props.data.data.quantity}</div>
        </Card>
    )
}
